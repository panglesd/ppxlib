...
let rec map_node_rec context ts super_call loc base_ctxt x =
  ...
  | Some (ext, attrs) -> (
      ...
      | None -> Ok (super_call base_ctxt x)
      | Some x ->
          let+ x = EC.merge_attributes_res context x attrs in
...
let handle_attr_group_inline attrs rf ~items ~expanded_items ~loc ~base_ctxt =
  ...
  List.fold_left attrs ~init:(Ok [])
    ~f:(fun acc (Rule.Attr_group_inline.T group) ->
      let+ acc = acc in
      let+ g1 = get_group group.attribute items in
      let+ g2 = get_group group.attribute expanded_items in
      match (g1, g2) with
      | None, None -> Ok acc
            ...
            | Pstr_extension (ext, attrs) -> (
                ...
                | Ok (Some items) ->
                    ...
                    if List.length no_attributes_errors = 0 then (
                      ...
                      |> List.map
                           ~f:
                             (EC.ext_item_from_context EC.Structure_item ~x:item)
                      )
                ...
                | Error err ->
                    ...
                    |> NonEmptyList.map ~f:Location.Error.to_extension
                    |> NonEmptyList.map
                         ~f:(EC.ext_item_from_context EC.Structure_item ~x:item)
                    |> NonEmptyList.to_list)
                    @ loop rest ~in_generated_code)
            | _ -> (
                ...
                | Pstr_type (rf, tds), Pstr_type (exp_rf, exp_tds) ->
                    ...
                    (let+ extra_items =
                       handle_attr_group_inline attr_str_type_decls rf
            ...
            | Psig_extension (ext, attrs) -> (
                ...
                | Ok (Some items) ->
                    ...
                    if List.length no_attributes_errors = 0 then (
                      ...
                      |> List.map
                           ~f:
                             (EC.ext_item_from_context EC.Signature_item ~x:item)
                      )
                      @ loop rest ~in_generated_code
                | Error err ->
                    (err
                    |> NonEmptyList.map ~f:Location.Error.to_extension
                    |> NonEmptyList.map
                         ~f:(EC.ext_item_from_context EC.Signature_item ~x:item)
                    |> NonEmptyList.to_list)
                    @ loop rest ~in_generated_code)
            | _ -> (
                let error_of_extension e =
                ...
                | Psig_type (rf, tds), Psig_type (exp_rf, exp_tds) ->
                    ...
                    assert (Poly.(rf = exp_rf));
                    (let+ extra_items =
                       handle_attr_group_inline attr_sig_type_decls rf
                         ~items:tds ~expanded_items:exp_tds ~loc ~base_ctxt
                     in
                     let+ expect_items =
